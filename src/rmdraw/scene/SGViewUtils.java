...
public class SGViewUtils {
    ...
    {
        double w = 0; for(SGView s : theViews) w += s.getWidth();
    ...
    {
        double my = 0; for(SGView s : theViews) if(s.isVisible()) my = Math.max(my, s.getFrameMaxY());
        return my;
    }
    ...
    { int c = compareViewFrameY(s0,s1); if(c==0) c = compareViewFrameX(s0,s1); return c; }
    ...
    public static SGPolygon getCombinedPathForViews(List <SGView> theViews)
    {
        ...
        SGView view0 = theViews.size()>0? theViews.get(0) : null; if(view0==null) return null;
        ...
        polyView.copyView(view0); polyView._rss = null;
