...
public class Tool<T extends SGView> extends ViewOwner {
    ...
    // Constants
    public static Border DEFAULT_BORDER = Border.blackBorder();
    public static boolean DEFAULT_PENCIL_ONESHOT = false;
    ...
    {
        return ClassUtils.newInstance(getViewClass());
    }
    ...
    {
        return getClass() == Tool.class ? new Label() : super.createUI();
    }
     ...
     */
    public EditorInteractor getEditorEvents()
    {
        return getEditor().getInteractor();
    }
     ...
     */
    public List<? extends SGView> getSelViews()
    {
        return getEditor().getSelOrSuperSelViews();
    }

    /**
     * Returns the tool for a given view.
     */
    public Tool getTool(SGView aView)
    {
     ...
     */
    public Tool getToolForClass(Class<? extends SGView> aClass)
    {
        return _editor.getToolForClass(aClass);
    }
    ...
    {
        return getEditor().isSelected(aView);
    }
    ...
    {
        return getEditor().isSuperSelected(aView);
    }
    ...
    {
        return aView.superSelectable();
    }

    /**
     * Returns whether a given view accepts children.
     */
    public boolean getAcceptsChildren(SGView aView)
    {
        return aView.acceptsChildren();
    }

    /**
     * Returns whether a given view can be ungrouped.
     */
    public boolean isUngroupable(SGView aView)
    {
        return aView.getChildCount() > 0;
    }
    ...
    public void processEvent(T aView, ViewEvent anEvent)
    {
        switch (anEvent.getType()) {
            case MousePress:
                mousePressed(aView, anEvent);
                break;
            case MouseDrag:
                mouseDragged(aView, anEvent);
                break;
            case MouseRelease:
                mouseReleased(aView, anEvent);
                break;
            case MouseMove:
                mouseMoved(aView, anEvent);
                break;
            default:
                if (anEvent.isKeyEvent()) processKeyEvent(aView, anEvent);
        }
    }
    ...
    public static class ViewHandle {
         ...
         * Creates ViewHandle.
         */
        public ViewHandle(SGView aView, int aHndl, Tool aTool)
        {
            view = aView;
            handle = aHndl;
            tool = aTool;
     ...
     */
    protected ViewEvent createSceneEvent(SGView s, ViewEvent e)
    {
        return getEditor().createSceneViewEvent(s, e, null);
    }
